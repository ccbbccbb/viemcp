...
async function fetchViemDocsTree(): Promise<string[]> {
  ...
  return data.tree
    .filter(
      (e) =>
        e.type === 'blob' && e.path.startsWith(base) && e.path.endsWith('.mdx'),
    )
  ...
  server.registerResource(
    ...
    async (url) => {
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `GitHub raw fetch failed: ${res.status} ${res.statusText}`,
          )
        }
        const text = await res.text()
        return {
          contents: [{ uri: url.href, mimeType: 'text/markdown', text }],
        }
      } catch (error) {
        return {
          contents: [
